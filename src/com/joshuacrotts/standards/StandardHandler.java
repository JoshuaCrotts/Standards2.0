...
/**
 * StandardHandler will be a class for a Handler that will loop through all
 * available StandardGameObjects, tick, and render them. They *must* be added in
 ...
 *
 * Other Handlers can be made of course, and one probably shouldn't extend this
 * class for a proprietary Handler for a class that, for example, will only
 * parse through, in example, StandardParticle objects.
 *
 * @author Joshua
 *
 */
public class StandardHandler
{
    ...
    /**
     * This method will tick any StandardGameObject that is passed to it Very
     * useful for abstraction, if needed.
     */
    ...
    /**
     * This method will tick any StandardHandler that is passed to it Very
     * useful for abstraction, if needed.
     */
    public static void Handler( StandardHandler handler )
    {
        /**
         * We're forced to parse through the entire list because entities is not
         * static; we can't simply do handler.tick() in this case. It still runs
         * at the same speed as tick() though.
         */
    ...
    /**
     * This method will tick every single object in the entities array list. An
     * entity is determined by an object or something in the game. Enemies,
     * Walls, Players, Other stuff is an entity. Ticking the entity ***will
     * update positions***, do stuff, etc. NOTHING GRAPHICALLY IS DONE IN THE
     * tick() METHOD.
     */
    public void tick()
    {
    ...
    /**
     * This method will render everything graphically per object. All objects
     * are entities that the render method is being called on. Rendering object
     * will update it's positions GRAPHICALLY.
     *
     * @param g is the graphics object. Just used. Yknow. NO ACTUAL MOVEMENTS OR
     * CALCULATIONS (setVelX(getVelX()+10)) ARE DONE IN THE render(Graphics g)
     * method.
     */
    ...
    /**
     * This will remove an entity based on the StandardGameObject passed.
     *
     * @param obj is a StandardGameObject, if found in ArrayList, it will be
     * removed.
     */
    ...
    /**
     * Clears all entities including the player. USE WITH CAUTION
     */
